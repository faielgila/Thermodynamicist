...
public abstract class CubicEquationOfState : EquationOfState
{
	...
	public override MolarVolume[] PhaseFinder(Temperature T, Pressure P)
	{
		/*
		...
		second turning point to "infinity" (this code stops at 1).
		...
		Once the turning point is calculated, the two turning points are found. Once the turning points are found,
		the bisection algorithm is applied to each range (excluding the root between the turning points, since it
		does not correspond to any real state).
		*/
		...
		*/
		...
		if (Math.Abs(f_L - f_V) < precisionLimit) return new[] { VMol_L, VMol_V };
		if (f_L > f_V) return new[] { 0, VMol_V };
		if (f_L < f_V) return new[] { VMol_L, 0 };
		
		// This statement should never be reached!
		// TODO: Throw an exception if this line is reached.
		return new MolarVolume[] { 0, 0 };
	}
